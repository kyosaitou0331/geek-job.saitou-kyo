 ...
 * @author owner
 */
public class zaikokanrisisutem extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           int data2=Integer.parseInt(request.getParameter("age"));
             try {
                  ...
                  db_st=db_co.prepareStatement("select * from human where humanID=? and name=? and age=?");
                  db_st.setInt(1,data1);
                  db_st.setString(2,request.getParameter("name"));
                  db_st.setInt(3,data2);
                  db_data=db_st.executeQuery();
                 
                  while(db_data.next()){
                     RequestDispatcher dispatch=request.getRequestDispatcher("zaikokanrisisutem.jsp");
